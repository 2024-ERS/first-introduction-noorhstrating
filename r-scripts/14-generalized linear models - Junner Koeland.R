...
anova(m1)
# same method using aov instead of lm
...
# is this better
AIC(m4, m3) # not really; the difference is not more than 2 - a normal distribution is also ok
...
m5 <- lm(data=dat, specrich~graztreat + block + graztreat:block)

# OR a shortcut
m5 <- lm(data=dat, specrich~graztreat*block)
anova(m5)
...
m8 <- lmerTest::lmer(specrich ~ graztreat + (graztreat|block), data=dat) # make it random by changing the 1|block into the whole graztreat
coef(m8) # coefficients 
summary(m8)
anova(m8)
