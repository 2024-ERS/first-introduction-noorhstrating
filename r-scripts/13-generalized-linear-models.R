...
summary(model1)
data$Value
coef(model1) # treatment effect of every block; slope of treatment is the same for every block
...
model2 <- lmerTest::lmer(Value ~ Treatment + (Treatment|Block), data = data) # replace 1 with treatment
summary(model2)
coef(model2) # treatment effect of every block; slope of treatment is the same for every block
ggplot(data, aes(x=Treatment, y=Value, color = Block)) +
  geom_jitter(width=0.15) +
...
# if at least value 2 lower, then often significantly different; the lower AIC, the better the model (more significant)
anova(model1, model2)
