...
p1 <- ggplot(data=dat2, aes(x=Point_ID, y=layer_thickness_cm, fill=soil_layer)) +
  geom_area() +
  coord_cartesian(ylim = c(95,125)) #change the scale of the graph
p1
...
####### select Limonium vulgare and analyze and plot its response to elevation, using logit
specsel <- "Specularia.marina"
p3 <- dat1 |>
  dplyr::filter(species==specsel) |>
  ggplot(aes(x=elevation_m, y=presence)) +
  geom_point(shape="|") +
  ggtitle(specsel) #title of the plot
p3



# calculate a logistic regression (also test quadratic term)
# this is a generalized linear model with logit link and binomial distribtion
m1 <- dat1 |> 
  dplyr::filter(species==specsel) |>
  glm(presence ~ elevation_m, family = binomial(link = logit), data=_) # transformation is logit, a function of link
...
m2 <- dat1 |> 
  dplyr::filter(species==specsel) |>
  glm(presence ~ elevation_m + I(elevation_m^2), #added quadratic term ^2
